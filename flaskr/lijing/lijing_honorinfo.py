...
def hello():
    ...
    flag_table = db.execute(
        "select honorinfo from year_list where year = ?", (year_list[0],)
    ).fetchone()["honorinfo"]
    if flag_table == 0:
        # sql_data = {}
        ...
        db.execute(
            "update year_list set honorinfo = 1 where year = ?",
...
def jsondata():
    ...
    try:
        sql = (
            "select p.person_id, p.person_name, count(h.person_id) num from person_"
            + session["year_current"]
            ...
            for i in range(1, table.nrows):
                ...
                for j in range(0, len(title_id)):
                    if title_id[j] == -1:
                        ...
                        insert_data[j] = float_int_string(row_value[title_id[j]])
                ...
                db.execute(
                    "insert into honor_"
                    + year_select
                    + " (person_id, school_name, honor_time, get_time, honor_unit, honor_name, honor_grade, honor_number, honor_remark) values (?,?,?,?,?,?,?,?,?)",
                    (
                        person_id,
                        insert_data[1],
                        insert_data[2],
...
def search():
    ...
    result = db.execute(
        "select school_name, honor_time, get_time, honor_unit, honor_name, honor_grade, honor_number, honor_remark from honor_"
        + session["year_current"]
        + " where person_id = ?",
        (person_id,),
    ).fetchall()
...
def add_data():
    if request.method == "POST":
        ...
        # 教师基本信息
        person_data = db.execute(
            "select person_id, person_name from person_" + session["year_current"]
        ).fetchall()
        person_id_dict = {}
        # person_name_list = []
        for i in person_data:
...
def update_data():
    ...
    db = get_lijing_db()
    honor_id_data = db.execute(
        "select honor_id from honor_"
        + session["year_current"]
        + " where person_id = ?",
        (person_id,),
    ).fetchall()
    ...
    person_name = db.execute(
        "select person_name from person_"
        + session["year_current"]
        + " where person_id = ?",
        (person_id,),
...
def search_data():
    ...
    sql = (
        ...
        + " as p LEFT JOIN honor_"
        + session["year_current"]
        + " as h on p.person_id = h.person_id where "
        + search_item
        + " like '%"
        + search_string
...
def exportData():
    ...
    sql = (
        "select p.person_name, school_name, honor_time, get_time, honor_unit, honor_name, honor_grade, honor_number, honor_remark from person_"
        + year
        + " as p left join honor_"
        + year
    ...
    if flag_search == "true":
        ...
        person_data = db.execute("select person_id from person_" + year).fetchall()
        for i in person_data:
