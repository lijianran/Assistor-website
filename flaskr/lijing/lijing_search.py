...
def hello():
    ...
    return render_template('lijing/search.html', year_list=year_list, school_list=school_list)
...
def search_data():
    ...
    if school == '全部数据':
        ...
        # 查特定分校
        person_id_data = select_table(['school', 'job'], year, {'person_id': 'job'}, {'school_'+year+'.school_name': ' = \''+school+'\''})
    ...
    # 写入表格数据
    item_name_dict = {
        'person_id': '编号', 'person_name': '姓名', 'gender': '性别', 'id_number': '身份证号', 'phone': '联系电话', 'political_status': '政治面貌', 'time_Party': '入党时间', 'time_work': '参加工作时间', 'address': '家庭住址', 'resume': '个人简历',
    ...
    return jsonify({'msg': msg, 'total': total, 'filename': 'exportData.xlsx'})