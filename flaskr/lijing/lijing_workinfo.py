...
def hello():
    ...
    flag_table = db.execute(
        "select workinfo from year_list where year = ?", (year_list[0],)
    ).fetchone()["workinfo"]
    if flag_table == 0:
        # sql_data = {}
        ...
        #     db.execute(sql)
        create_table(
            ["school", "job", "class", "rank", "workload"], session["year_current"]
        )
        ...
        db.execute(
            "update year_list set workinfo = 1 where year = ?",
            (session["year_current"],),
        )
        db.commit()
    ...
    school_data = db.execute(
        "select school_id, school_name from school_" + session["year_current"]
    ).fetchall()
...
def jsondata():
    ...
    person_list = []
    person_id_list = []
    person_data = db.execute("select person_id, person_name from " + person).fetchall()
    ...
    for i in person_id_list:
        ...
        if school_data is None:
            db.execute(
                "insert into "
                + job
                + " (job_name, school_id, person_id) values (?,?,?)",
                (
                    "无",
                    1,
                    i,
...
def add_school():
    ...
    school_data = db.execute("select school_name from school_" + year_select).fetchall()
    school_list = []
    ...
    sql_school = "insert into school_" + year_select + " (school_name) values (?)"
    db.execute(sql_school, (school_name,))
    db.commit()

    school_dict = {}
    school_data = db.execute(
        "select school_id, school_name from school_" + year_select
    ).fetchall()
    for i in school_data:
    ...
    for i in school_dict:
        class_data = db.execute(
            "select class_name from class_" + year_select + " where school_id = ?",
            (school_dict[i],),
        ).fetchall()
        clas = []
        for j in class_data:
...
def add_class():
    ...
    school_id = db.execute(
        "select school_id from school_" + year_select + " where school_name = ?",
        (school_name,),
    ).fetchone()["school_id"]
    person_data = db.execute(
        "select person_id from person_" + year_select + " where person_name = ?",
        (person_name,),
    ).fetchone()
    class_data = db.execute(
        "select class_name from class_" + year_select + " where school_id = ?",
    ...
    return jsonify(
        {"msg": "成功添加班级", "school_list": school_list, "class_list": class_lists}
    )
...
def del_school():
    ...
    if (
        db.execute(
            "select person_id from job_" + year_select + " where school_id = ?",
            (school_id,),
        ).fetchone()
        is not None
    ):
        return jsonify(
            {"msg": "error", "error": "存在教师属于分校 “" + school_name + "” ，删除分校失败"}
        )

    sql_class = "delete from class_" + year_select + " where school_id = ?"
    sql_school = "delete from school_" + year_select + " where school_name = ?"
...
def del_class():
    ...
    if class_id_data is None:
        return jsonify(
            {
                "msg": "error",
                "error": "分校 “" + school_name + "” 中不存在" + class_name + "班，删除班级失败",
            }
        )
    ...
    rank_data = db.execute(
        "select person_id from rank_" + year_select + " where class_id = ?", (class_id,)
    ).fetchone()
    if rank_data is not None:
        return jsonify(
            {"msg": "error", "error": "存在教师在班级 “" + class_name + "” 中任课，删除班级失败"}
        )

    sql_class = "delete from class_" + year_select + " where class_id = ?"
...
def importData():
    if request.method == "POST":
        ...
        person_data = db.execute(
            "select person_id, person_name from person_" + year_select
        ).fetchall()
        person_id_dict = {}
        person_name_list = []
        for i in person_data:
        ...
        for i in range(1, table.nrows):
            ...
            if "lesson_number" in update_item:
                lesson_number = float_int_string(row[update_item["lesson_number"]])
                if (
                    db.execute(
                        "select workload_id from workload_"
                        + year_select
                        + " where person_id = ?",
                        (person_id,),
                    ).fetchone()
                    ...
                    db.execute(
                        "update workload_"
                        + year_select
                        + " set lesson_number = ? where person_id = ?",
                        (
                            lesson_number,
                            person_id,
                        ),
            ...
            if "year_result" in update_item:
                ...
                if (
                    ...
                    db.execute(
                        "update workload_"
                        + year_select
                        + " set year_result = ? where person_id = ?",
                        (
                            year_result,
                            person_id,
                        ),
            ...
            if "class_id" in update_item:
                ...
                if class_id_data is None:
                    return jsonify(
                        {"msg": "分校“" + school_name + "”中不存在班级：“" + class_name + "”"}
                    )
                class_id = class_id_data["class_id"]

                subject = row[update_item["subject"]]
                rank_data = db.execute(
                    "select rank_up_school from rank_"
                    + year_select
                    + " where person_id = ? and subject = ? and class_id = ?",
                    (
                        person_id,
                        subject,
                        class_id,
            ...
            if "rank_up_school" in update_item:
                rank_up_school = float_int_string(row[update_item["rank_up_school"]])
                db.execute(
                    "update rank_"
                    + year_select
                    + " set rank_up_school = ? where subject = ? and person_id = ? and class_id = ?",
                    (
                        rank_up_school,
                        subject,
                        person_id,
            ...
            if "rank_up_country" in update_item:
                rank_up_country = float_int_string(row[update_item["rank_up_country"]])
                db.execute(
                    "update rank_"
                    + year_select
                    + " set rank_up_country = ? where subject = ? and person_id = ? and class_id = ?",
                    (
                        rank_up_country,
                        subject,
                        person_id,
            ...
            if "rank_down_school" in update_item:
                ...
                db.execute(
                    "update rank_"
                    + year_select
                    + " set rank_down_school = ? where subject = ? and person_id = ? and class_id = ?",
                    (
                        rank_down_school,
                        subject,
                        person_id,
            ...
            if "rank_down_country" in update_item:
                ...
                db.execute(
                    "update rank_"
                    + year_select
                    + " set rank_down_country = ? where subject = ? and person_id = ? and class_id = ?",
                    (
                        rank_down_country,
                        subject,
                        person_id,
...
def exportData():
    ...
    rank = "rank_" + year
    sql_search = (
        "select p.person_name, class.class_name, school.school_name, job.job_name, work.lesson_number, work.year_result from "
        + person
        + " as p left join "
        + clas
        + " as class on p.person_id = class.person_id left join "
        + job
        + " as job on p.person_id = job.person_id left join "
        + workload
        + " as work on p.person_id = work.person_id left join "
        + school
        + " as school on job.school_id = school.school_id where p.person_id = ?"
    )
    sql_rank = (
        "select subject, class_name, rank_up_school, rank_up_country, rank_down_school, rank_down_country from "
        + rank
        + " as r join "
        + clas
        + " as c on r.class_id = c.class_id where r.person_id = ?"
    )
    print(sql_rank)
    ...
    rank_name_list = ["任教班级", "任教学科", "上学期全校排名", "上学期全县排名", "下学期全校排名", "下学期全县排名"]
...
def search():
    ...
    person_name = db.execute(
        "select person_name from person_" + year + " where person_id = ?", (person_id,)
    ).fetchone()["person_name"]
    job_data = db.execute(
        "select school_id, job_name from job_" + year + " where person_id = ?",
        (person_id,),
    ).fetchone()
    school_id = job_data["school_id"]
    job_name = job_data["job_name"]
    school_name = db.execute(
        "select school_name from school_" + year + " where school_id = ?", (school_id,)
    ).fetchone()["school_name"]

    workload_data = db.execute(
        "select lesson_number, year_result from workload_"
        + year
        + " where person_id = ?",
        (person_id,),
    ).fetchone()
    ...
    class_data = db.execute(
        "select class_name from class_" + year + " where person_id = ?", (person_id,)
    ).fetchone()
    ...
    rank_data = db.execute(
        "select subject, class_id, rank_up_school, rank_up_country, rank_down_school, rank_down_country from rank_"
        + year
        + " where person_id = ?",
        (person_id,),
    ).fetchall()
    ...
    for i in rank_data:
        ...
        d.append(
            db.execute(
                "select class_name from class_" + year + " where class_id = ?",
                (i["class_id"],),
            ).fetchone()["class_name"]
        )
        d.append(i["subject"])
...
def update_data():
    ...
    school_id = school_id_data["school_id"]
    db.execute(
        "update job_" + year_select + " set school_id = ? where person_id = ?",
        (
            school_id,
            person_id,
        ),
    ...
    db.execute(
        "update job_" + year_select + " set job_name = ? where person_id = ?",
        (
            job_name,
            person_id,
        ),
    ...
    if (
        db.execute(
            "select workload_id from workload_" + year_select + " where person_id = ?",
            (person_id,),
        ).fetchone()
        is None
    ):
    ...
    db.execute(
        "update workload_" + year_select + " set year_result = ? where person_id = ?",
        (
            year_result,
            person_id,
        ),
    ...
    for rank in rank_data:
        ...
        if class_id_data is None:
            return jsonify(
                {
                    "msg": "error",
                    "error": "分校“" + school_name + "”中不存在班级：“" + class_name + "”",
                }
            )
        class_id = class_id_data["class_id"]
        ...
        if rank_data is None:
            ...
            db.execute(
                "update rank_"
                + year_select
                + " set rank_up_school = ?, rank_up_country = ?, rank_down_school = ?, rank_down_country = ? where person_id = ? and subject = ? and class_id = ?",
                (
                    rank[2],
                    rank[3],
                    rank[4],
...
def delete_data():
    ...
    class_id = db.execute(
        "select class_id from class_" + year + " where class_name = ?", (class_name,)
    ).fetchone()["class_id"]

    db.execute(
        "delete from rank_"
        + year
        + " where person_id = ? and class_id = ? and subject = ?",
        (
            person_id,
            class_id,
            subject,
    ...
    return jsonify(
        {"msg": "成功删除记录（" + person_name + "，" + class_name + "班，" + subject + "）"}
    )
...
def search_data():
    ...
    if search_school != "#全部数据":
        person_data = db.execute(
            "select person_"
            + year
            + ".person_id, person_name from job_"
            + year
            + " join school_"
            + year
            + ", person_"
            ...
            + ".person_id = person_"
            + year
            + ".person_id where school_name = ?",
            (search_school,),
        ).fetchall()
    else:
        person_data = db.execute(
            "select person_id, person_name from person_" + year
        ).fetchall()
    ...
    if search_item == "school_name":
        ...
        result = db.execute(
            "select person_id from job_" + year + " where school_id = ?",
            (school_data["school_id"],),
        ).fetchall()

    if search_item == "job_name":
        result = db.execute(
            "select person_id from job_"
            + year
            + ' where job_name like "%'
            + search_string
            + '%"'
    ...
    if search_item == "lesson_number":
        result = db.execute(
            "select person_id from workload_"
            + year
            + ' where lesson_number like "%'
            + search_string
            + '%"'
        ).fetchall()
    ...
    if search_item == "class_name":
        ...
        if len(class_data) == 0:
            ...
            result = db.execute(
                "select person_id from rank_" + year + " where class_id = ?",
                (class_data[0]["class_id"],),
            ).fetchall()
        else:
            if search_school == "#全部数据":
                ...
                class_id = db.execute(
                    ...
                    + ".school_id = school_"
                    + year
                    + '.school_id where class_name like "%'
                    + search_string
                    + '%"'
                ).fetchone()["class_id"]
                result = db.execute(
